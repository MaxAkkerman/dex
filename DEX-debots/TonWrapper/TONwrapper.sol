...
contract TONWrapper is ITONWrapper {
	...
	function createZeroWallet() public view checkOwnerAndAccept {
		if (initStatus == false) {
			...
			TvmCell body = tvm.encodeBody(IRootTokenContract(creator).deployEmptyWallet, setZeroWallet, 0, walletId, ownerUINT, GRAMS_CREATE_NEWWALLET);
	...
	function createDepositWallet(address rootAddr) private pure inline {
		...
		address owner = address(this);
		uint256 ownerUINT = owner.value;
		TvmCell body = tvm.encodeBody(IRootTokenContract(creator).deployEmptyWallet, setDepositWallet, 0, walletId, ownerUINT, GRAMS_CREATE_NEWWALLET);
		creator.transfer({value:GRAMS_CREATE_ROOT, bounce:false, body:body});
	}
	...
	function unwrapGrams() public alwaysAccept override functionID(0x00000052) {
		...
		TvmCell bodyA = tvm.encodeBody(ITONTokenWallet(depositwallet).getBalance_InternalOwner, balanceDepositWallet);
